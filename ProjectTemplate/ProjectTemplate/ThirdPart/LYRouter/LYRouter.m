...
static NSString * const LY_ROUTER_WILDCARD_CHARACTER = @"~";
static NSString *specialCharacters = @"/?&.";

NSString *const LYRouterParameterURL = @"LYRouterParameterURL";
NSString *const LYRouterParameterCompletion = @"LYRouterParameterCompletion";
NSString *const LYRouterParameterUserInfo = @"LYRouterParameterUserInfo";
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion
{
    ...
    if (parameters) {
        ...
        if (completion) {
            parameters[LYRouterParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[LYRouterParameterUserInfo] = userInfo;
        }
        if (handler) {
...
{
    LYRouter *router = [LYRouter sharedInstance];
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url
{
    ...
    for (NSString* pathComponent in pathComponents) {
        ...
        for (NSString* key in subRoutesKeys) {
            if ([key isEqualToString:pathComponent] || [key isEqualToString:LY_ROUTER_WILDCARD_CHARACTER]) {
                found = YES;
...
- (NSArray*)pathComponentsFromURL:(NSString*)URL
{
    ...
    if ([URL rangeOfString:@"://"].location != NSNotFound) {
        ...
        if ((pathSegments.count == 2 && ((NSString *)pathSegments[1]).length) || pathSegments.count < 2) {
            [pathComponents addObject:LY_ROUTER_WILDCARD_CHARACTER];
        }
