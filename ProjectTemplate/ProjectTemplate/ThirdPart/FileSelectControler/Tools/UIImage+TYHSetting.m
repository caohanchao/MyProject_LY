...
+ (UIImage *)imageWithFileName:(NSString *)filename {
    ...
    if([textTypesArray containsObject:[filename pathExtension]]){
//        model.fileType = MKFileTypeTxt;
        if (![[filename pathExtension] compare:@"xls" options:NSCaseInsensitiveSearch] || ![[filename pathExtension] compare:@"xlsx" options:NSCaseInsensitiveSearch]) {
        ...
        if (![[filename pathExtension] compare:@"txt" options:NSCaseInsensitiveSearch]) {
            return[UIImage imageNamed:@"txt-文档"];
        }
        if (![[filename pathExtension] compare:@"doc" options:NSCaseInsensitiveSearch] || ![[filename pathExtension] compare:@"docx" options:NSCaseInsensitiveSearch]) {
            return [UIImage imageNamed:@"word-文档"];
        }
    }else if([viceViodeArray containsObject: [filename pathExtension]] || [AVArray containsObject:[filename pathExtension]]){
//        model.fileType = MKFileTypeAudioVidio;
        if ([viceViodeArray containsObject: [filename pathExtension]]) {
            return[UIImage imageNamed:@"音乐"];
        }else{
            return [UIImage imageNamed:@"视频"];
        }
    }else if([appViodeArray containsObject: [filename pathExtension]]){
//        model.fileType = MKFileTypeApplication;
        return  [UIImage imageNamed: @"未知问题-本机"];
    }else{
//        model.fileType = MKFileTypeUnknown;
        if (![[filename pathExtension] compare:@"rar" options:NSCaseInsensitiveSearch] || ![[filename pathExtension] compare:@"zip" options:NSCaseInsensitiveSearch]) {
            return  [UIImage imageNamed:@"rar-其他"];
        } else if (![[filename pathExtension] compare:@"htlm" options:NSCaseInsensitiveSearch]){
            return  [UIImage imageNamed:@"rar-其他"];
        } else{
            return  [UIImage imageNamed: @"未知问题-本机"];
        }
