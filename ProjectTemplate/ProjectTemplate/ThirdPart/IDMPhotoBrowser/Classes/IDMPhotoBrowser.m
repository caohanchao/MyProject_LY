...
// Private
@interface IDMPhotoBrowser ()<UINavigationControllerDelegate> {
	// Data
...
@property (nonatomic, assign) BOOL firistAppear;// 判断是否第一次进入
// Private Methods
...
//    CGRect senderViewOriginalFrame = _senderViewForAnimation.superview ? [_senderViewForAnimation.superview convertRect:_senderViewForAnimation.bounds toView:_applicationWindow] : _senderViewOriginalFrame;
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(back) name:IDMPhotoBrowserDismissNotification object:nil];
    //转发图片
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(pushToForwardVC:) name:ImageForwardPostNotification object:nil];
    // Update
    //[self reloadData];
...
- (void)back {
    ...
    [self dismissViewControllerAnimated:NO completion:^{
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ChatControllerBackNotification" object:nil];
    }];
}
...
- (void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    else if ([viewController isKindOfClass:[UserDesInfoController class]]){
        isHomePage = YES;
    }
...
-(void)LongPressToDO:(UILongPressGestureRecognizer *)longPress{
    if (longPress.state==UIGestureRecognizerStateBegan) {
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            if (self.longPressGesResponse) {
                self.longPressGesResponse(image);
            }
        }
    }
}

- (void)pushToForwardVC:(NSNotification *)text
{
    ChatMessageForwardController *forwardVC =[[ChatMessageForwardController alloc]init];
    forwardVC.pushViewStr = @"postDetailView";
    forwardVC.imageUrlStr = text.userInfo[@"imageUrl"];
