...
- (BOOL)requestAuthorization:( void(^)())cancelBlock
{
    BOOL ret = YES;
...
//                [alertView show];
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示" message:@"请在隐私设置中打开定位开关" preferredStyle:UIAlertControllerStyleAlert];
 ...
 */
- (void)geocode:(NSString *)address success:(void(^)(AMapPOI *pm))success failure:(void(^)())failure{
    [self.geocoder geocodeAddressString:address completionHandler:^(NSArray *pois, NSError *error) {
        if (error) {
            if (failure) {
                failure();
            }
 ...
 *  @param failure       失败block
 */
- (void)reverseGeocodeWithCoordinate2D:(CLLocationCoordinate2D)coordinate2D success:(void(^)(NSArray *pois))success failure:(void(^)())failure{
    CLLocation *location = [[CLLocation alloc] initWithLatitude:coordinate2D.latitude longitude:coordinate2D.longitude];
//    [self.geocoder reverseGeocodeLocation:location completionHandler:^(NSArray *pois, NSError *error) {
//        if (error) {
//            if (failure) {
//                failure();
//            }
