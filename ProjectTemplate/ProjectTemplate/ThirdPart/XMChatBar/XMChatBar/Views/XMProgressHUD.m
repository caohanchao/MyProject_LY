...
- (void)show{
    self.angle = 0.0f;
    self.seconds = 0;
    self.subTitleLabel.text = @"向上滑动取消录音";
...
- (void)setSubTitle:(NSString *)subTitle{
    ...
    if ([subTitle isEqualToString:@"松开手指取消录音"]) {
        self.subTitleLabel.backgroundColor = RGB(148, 49, 48);
    }else if ([subTitle isEqualToString:@"向上滑动取消录音"]) {
...
- (UILabel *)subTitleLabel{
    if (!_subTitleLabel) {
        CGFloat w = [LZXHelper textWidthFromTextString:@" 向上滑动取消录音 " height:25 fontSize:14];
        _subTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, w, 25)];
