...
@implementation CHCBlurImageEffects

+ (UIImage *)pd_imageEffectLightFromImage:(UIImage *)inputImage
{
    UIColor *tintColor = [UIColor colorWithWhite:1.0 alpha:0.3];
    return [self pd_imageToImage:inputImage withRadius:60 tintColor:tintColor saturationDeltaFactor:1.8];
}

+ (UIImage *)pd_imageEffectExtraLightFromImage:(UIImage *)inputImage
{
    UIColor *tintColor = [UIColor colorWithWhite:0.97 alpha:0.82];
    return [self pd_imageToImage:inputImage withRadius:40 tintColor:tintColor saturationDeltaFactor:1.8];
}
...
{
    const CGFloat colorAlpha = 0.6;
    UIColor *effectColor = tintColor;
    size_t componentCount = CGColorGetNumberOfComponents(tintColor.CGColor);
    if (componentCount == 2) {
        CGFloat b;
        if ([tintColor getWhite:&b alpha:NULL]) {
            effectColor = [UIColor colorWithWhite:b alpha:colorAlpha];
        }
    } else {
        CGFloat r, g, b;
        if ([tintColor getRed:&r green:&g blue:&b alpha:NULL]) {
            effectColor = [UIColor colorWithRed:r green:g blue:b alpha:colorAlpha];
        }
    }
    return [self pd_imageToImage:inputImage withRadius:20 tintColor:effectColor saturationDeltaFactor:-1];
}

#pragma mark - 实现方法
+ (UIImage *)pd_imageToImage:(UIImage*)inputImage withRadius:(CGFloat)blurRadius tintColor:(UIColor *)tintColor saturationDeltaFactor:(CGFloat)saturationDeltaFactor
{
