...
//        chineseString.string =[ChineseString RemoveSpecialCharacter:chineseString.string];
    ...
    for(int i=0;i<[chineseStringsArray count];i++){
        //将###的pinYin移到最后
        ChineseString *tmpString = [chineseStringsArray objectAtIndex:i];
        NSString *firSubString = [tmpString.pinYin substringToIndex:1];
        if ([firSubString isEqualToString:@"#"]) {
            [tmpArr addObject:tmpString];
        }
    }
    [chineseStringsArray removeObjectsInArray:tmpArr];
    [chineseStringsArray addObjectsFromArray:tmpArr];
