...
+ (HYBURLSessionTask *)_requestWithUrl:(NSString *)url
  ...
  NSString *absolute = [self absoluteUrlWithPath:url];
    ...
    if([params isKindOfClass:[NSMutableDictionary class]]){
        NSMutableDictionary *temp = (NSMutableDictionary *)params;
        temp[@"osType"] = @"ios";
    }
...
+ (AFHTTPSessionManager *)manager {
  @synchronized (self) {
    ...
    if (sg_sharedManager == nil || sg_isBaseURLChanged) {
      ...
      [manager setSecurityPolicy:[LZXHelper customSecurityPolicy]];
...
+ (NSString *)generateGETAbsoluteURL:(NSString *)url params:(id)params {
  if (params == nil || ![params isKindOfClass:[NSDictionary class]] || [(NSMutableArray *)params count] == 0) {
    return url;
  }
