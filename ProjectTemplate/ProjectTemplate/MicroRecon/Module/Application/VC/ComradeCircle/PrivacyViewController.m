...
float _lastPrContentOffset;

@interface PrivacyViewController ()<SqureCellDelegate,HZPhotoBrowserDelegate>
...
- (void)viewDidLoad {
    ...
    [self newPrivacyPostList];
    
    [self createMJRefresh];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(privacyPostList) name:PrivacyPostNotification object:nil];
     [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(newPrivacyPostList) name:NewPrivacyPostNotification object:nil];
    
}

- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView{
    _lastPrContentOffset = scrollView.contentOffset.y;
}

- (void)scrollViewWillBeginDecelerating:(UIScrollView *)scrollView{
    if (!(_lastPrContentOffset < scrollView.contentOffset.y))
    {
        NSLog(@"向下滚动");
...
//通知刷新页面
-(void)privacyPostList
{
    _modelArray =  [[[DBManager sharedManager] privacyPostListSQ] selectPrivacyPostList];
    [self.tableView reloadData];
}
//通知重新获取数据
-(void)newPrivacyPostList
{
    [self request_data];
}
...
{
    static NSString *privacyIdentifier = @"privacyIdentifier";
    SqureCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    if (cell == nil) {
        cell = [[SqureCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:privacyIdentifier];
    }
    else
    {
    ...
    PrivacyViewController  __weak * weakSelf = self;
...
- (void)request_data {
    ...
    [[HttpsManager sharedManager] post:GetCircleList parameters:paramDict  progress:^(NSProgress * _Nonnull progress) {
        ...
        [[[DBManager sharedManager] privacyPostListSQ]clearPrivacyPostPost];
        
        [[[DBManager sharedManager] privacyPostListSQ]  transactionInsertPostListModel:_postModel.results];
        _modelArray = [[[DBManager sharedManager]privacyPostListSQ]selectPrivacyPostList];
