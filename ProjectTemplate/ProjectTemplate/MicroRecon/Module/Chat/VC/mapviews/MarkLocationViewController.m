...
@interface MarkLocationViewController ()<UITableViewDelegate,UITableViewDataSource,MAMapViewDelegate,AMapSearchDelegate>
...
-(void)createNav
{
    ...
    UIBarButtonItem *rightItem =[[UIBarButtonItem alloc] initWithTitle:@"确定" style:UIBarButtonItemStylePlain target:self action:@selector(determine)];
    ...
    [rightItem setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIFont systemFontOfSize:16],NSFontAttributeName, nil] forState:UIControlStateNormal];
    
    self.navigationItem.leftBarButtonItems = [self.navigationItem leftItemsWithBarButtonItem:leftItem WithSpace:5];
...
//            [self.delegate sendLocation:locationUrl locationText:poi.address];
        ...
        param[@"longitude"] = [NSString stringWithFormat:@"%.8lf",self.mapView.centerCoordinate.longitude];
        param[@"latitude"] = [NSString stringWithFormat:@"%.8lf",self.mapView.centerCoordinate.latitude];
        param[@"position"] = poi.address;
        
        self.locationBlock(param);
